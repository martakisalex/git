...
Add or create some file such as git.md in the directory
...
Say we are done working with git.md. We can add it to the Staging Environment:
...
New Git Branch
Let's add some content to our git.md page.
...
Now we created a new branch called "git-branch-tutorial"

Let's confirm that we have created a new branch:

$ git branch

We can see the new branch with the name "git-branch-tutorial", but the * beside master specifies that we are currently on that branch.
...
There are changes to our git.md, but the file is not staged for commit
...
We are currently on the branch git-branch-tutorial. We added content to this branch, so let's list the files in the current directory:
...
The updated git.md is not a part of this branch. List the files in the current directory again:
